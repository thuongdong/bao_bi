	...
	class tp_megamenu {
		...
		function tp_menu_header() {
			if ( basename( $_SERVER['PHP_SELF'] ) == "nav-menus.php" ) {
				...
				wp_enqueue_script( 'tp-mega-menu-js', TP_FRAMEWORK_LIBS_URI . 'megamenu/js/tp-mega-menu.js', array( 'jquery', 'jquery-ui-sortable' ), false, true );
...
//			}
		...
		function update_menu( $menu_id, $menu_item_db ) {
			$check = apply_filters( 'avf_mega_menu_post_meta_fields', array( 'megamenu', 'item-style', 'menu-icon', 'bg-color', 'bg-image', 'hide-text', 'disable-link', 'full-width-dropdown', 'submenu-columns', 'side-dropdown-elements', 'submenu-type', 'widget-area', 'bg-image-repeat', 'bg-image-attachment', 'bg-image-position', 'bg-image-size' ), $menu_id, $menu_item_db );
			...
			if ( !self::$added_html_iconbox ) {
				...
				<script type="text/javascript">
					...
					function onSearchIconBoxKeyUp(filter) {
						var icons_str = "<?php echo $icons_str; ?>";
						if (!filter) {
							jQuery("#icon-dropdown .icon-list li").show();
						}
						var cmd = 'var match = icons_str.match(/[^\\s]?' + filter + '[^\\s]*/gi)';
						eval(cmd);
						if (filter.length) {
			...
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
                    ...
                    <span class="item-controls">
                        ...
                        <a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e( 'Edit Menu Item', 'thim' ); ?>" href="<?php
				...
				<div class='tp_mega_menu_options'>
					...
					<p class="description description-wide tp_checkbox tp_mega_menu tp_mega_menu_d1">
						<label for="edit-<?php echo $key . '-' . $item_id; ?>">
							...
							<input type="text" value="<?php echo $value; ?>" id="edit-<?php echo $key . '-' . $item_id; ?>" <?php
							?>class=" <?php echo $key; ?>" name="<?php echo $key . "[" . $item_id . "]"; ?>" />
							...
							?>title="<?php esc_attr_e( 'Click to browse icon', 'thim' ) ?>" <?php
								   ...
								   onclick="on_open_icon_browser('edit-<?php echo $key . '-' . $item_id; ?>');" />
					...
					<p class="description description-wide">
						<label for="edit-<?php echo $key . '-' . $item_id; ?>">
							<input type="checkbox" value="active" id="edit-<?php echo $key . '-' . $item_id; ?>" class=" <?php echo $key; ?>" name="<?php echo $key . "[" . $item_id . "]"; ?>" <?php echo $value; ?> /><?php esc_attr_e( $title ); ?>
						</label>
					</p>
					...
					if ( !$depth ) {
						...
						<p class="description description-wide tp_checkbox tp_mega_menu tp_mega_menu_d2"<?php echo $style; ?>>
							<label for="edit-<?php echo $key . '-' . $item_id; ?>">
								<span class='tp_long_desc'><?php esc_attr_e( $title ); ?></span><br />
								<input class="megamenu-colorpicker" type="text" name="<?php echo $key . "[" . $item_id . "]"; ?>" id="edit-<?php echo $key . '-' . $item_id; ?>" value="<?php echo $value; ?>" data-default-color="<?php echo $value; ?>" />
							</label>
						</p>
						...
						<p class="description description-wide tp_checkbox tp_mega_menu tp_mega_menu_d2"<?php echo $style; ?>>
							<label for="edit-<?php echo $key . '-' . $item_id; ?>">
								...
								<button id="browse-edit-<?php echo $key . '-' . $item_id; ?>"
										class="set_custom_images button button-secondary submit-add-to-menu"
										onclick="open_media_browser('edit-<?php echo $key . '-' . $item_id; ?>'); return false;"><?php esc_attr_e( 'Browse Image', 'thim' ); ?></button>
							</label>
						</p>
